  ...
  return (
    ...
    >
        ...
        <svg className="w-[500px] h-[500px] md:w-[600px] md:h-[600px]" viewBox="0 0 700 700" style={{ filter: 'drop-shadow(0 0 60px rgba(201,169,98,0.15))' }}>
          ...
          <g style={{ 
            ...
            {Array.from({ length: 12 }).map((_, i) => {
              ...
              return (
                <text key={i} x={x} y={y} fill="#3a3530" fontSize="12" fontFamily="Cinzel" textAnchor="middle" dominantBaseline="middle"
                  opacity={phase >= 1 ? 0.4 : 0} style={{ transition: 'opacity 2s ease', transitionDelay: `${i * 0.15}s` }}>
                  {runes[i]}
                </text>
              );
            })}
          ...
          <g style={{ 
            ...
            <polygon points="350,120 570,280 500,550 200,550 130,280" fill="none" stroke="#2a2520" strokeWidth="0.6" />
          </g>
          ...
          <g opacity={phase >= 2 ? 0.5 : 0} style={{ transition: 'opacity 2s ease' }}>
            <polygon points="350,80 580,250 500,580 200,580 120,250" fill="none" stroke="url(#goldGrad)" strokeWidth="1.5" filter="url(#softGlow)"
              style={{ strokeDasharray: 2000, strokeDashoffset: phase >= 2 ? 0 : 2000, transition: 'stroke-dashoffset 3s ease-out' }} />
          </g>
      ...
      <div className="relative z-10 text-center pointer-events-none">
        <div className="mb-12 relative" style={{ opacity: phase >= 3 ? 1 : 0, transform: phase >= 3 ? 'scale(1)' : 'scale(0.7)', transition: 'all 1.5s cubic-bezier(0.16, 1, 0.3, 1)' }}>
          <div className="absolute inset-0 rounded-full transition-opacity duration-1000"
            style={{ background: 'radial-gradient(circle, rgba(201,169,98,0.15) 0%, transparent 70%)', width: '150px', height: '150px', left: '50%', top: '50%', transform: 'translate(-50%, -50%)', opacity: phase >= 4 ? 1 : 0.5, animation: phase >= 4 ? 'breathe 4s ease-in-out infinite' : 'none' }} />
        ...
        <div className="relative">
          <h1 className="text-6xl md:text-8xl font-cinzel tracking-[0.6em] text-transparent bg-clip-text relative"
            style={{ opacity: phase >= 3 ? 1 : 0, backgroundImage: 'linear-gradient(180deg, #E8D5A3 0%, #C9A962 30%, #8B7355 60%, #5a4a3a 100%)', textShadow: phase >= 4 ? '0 0 80px rgba(201,169,98,0.4), 0 0 160px rgba(201,169,98,0.2)' : '0 0 40px rgba(201,169,98,0.2)', transition: 'all 2s ease', letterSpacing: phase >= 4 ? '0.8em' : '0.6em' }}>
            {text || '\u00A0\u00A0\u00A0\u00A0'}
          </h1>
          <p className="mt-8 text-xs uppercase tracking-[0.5em] font-montserrat" style={{ opacity: phase >= 4 ? 0.5 : 0, color: '#8B7355', transition: 'opacity 2s ease 0.5s' }}>
            Western Occult Apparel
          </p>
        </div>
        
        <div className="flex gap-6 justify-center mt-16" style={{ opacity: phase < 5 ? 0.3 : 0, transition: 'opacity 1s' }}>
          {[0, 1, 2, 3, 4].map((i) => (
            <div key={i} className="w-1 h-1 rounded-full transition-all duration-700"
              style={{ backgroundColor: phase >= i ? '#C9A962' : '#2a2520', transform: phase === i ? 'scale(1.8)' : 'scale(1)', opacity: phase >= i ? 1 : 0.3 }} />
          ))}
        </div>
      </div>

      <button onClick={onComplete} className="fixed bottom-10 right-10 text-zinc-700 text-[10px] font-montserrat uppercase tracking-[0.3em] hover:text-[#C9A962] transition-all duration-500 z-[101] group">
        <span className="relative">Skip<span className="absolute -bottom-1 left-0 w-0 h-px bg-[#C9A962] group-hover:w-full transition-all duration-500" /></span>
      </button>
...
// --- HERO SECTION ---
const Hero = ({ onEnter, visible }: { onEnter: () => void; visible: boolean }) => {
  const [mousePos, setMousePos] = useState({ x: 0, y: 0 });
  ...
  return (
    <section 
      ...
      style={{ 
        background: 'radial-gradient(ellipse at center, #0f0e0d 0%, #050505 40%, #010101 100%)',
        opacity: visible ? 1 : 0,
    ...
    >
      ...
      <div className="z-10 text-center flex flex-col items-center"
        ...
        <div className="absolute w-[400px] h-[150px] rounded-full pointer-events-none transition-opacity duration-1000"
          style={{ background: 'radial-gradient(ellipse, rgba(201,169,98,0.08) 0%, transparent 70%)', opacity: contentOpacity }} />
        
        <h1 className="text-6xl md:text-8xl font-cinzel font-normal tracking-[0.4em] text-transparent bg-clip-text mb-8"
          style={{
            backgroundImage: 'linear-gradient(180deg, #E8D5A3 0%, #C9A962 40%, #8B7355 100%)',
            textShadow: '0 0 60px rgba(201,169,98,0.2)',
            opacity: contentOpacity,
            transform: contentOpacity ? 'translateY(0)' : 'translateY(30px)',
            transition: 'opacity 1.5s ease 0.3s, transform 1.5s ease 0.3s',
          }}>
          ZEAL
        </h1>
        
        <div className="relative mb-12 px-10 py-4 border border-[#C9A962]/20 bg-black/20"
          style={{ opacity: contentOpacity, transform: contentOpacity ? 'translateY(0)' : 'translateY(20px)', transition: 'all 1.5s ease 0.6s' }}>
          <p className="text-[#8B7355] font-cinzel tracking-[0.4em] text-[10px] uppercase">As Above • So Below</p>
        ...
        <p className="text-zinc-700 font-montserrat tracking-[0.3em] text-[10px] uppercase mb-16"
          style={{ opacity: contentOpacity, transition: 'opacity 1.5s ease 0.9s' }}>
          Western Occult Apparel
        </p>
        ...
        <button onClick={onEnter}
          ...
          style={{ opacity: contentOpacity, transform: contentOpacity ? 'translateY(0)' : 'translateY(20px)', transition: 'opacity 1.5s ease 1.2s, transform 1.5s ease 1.2s, border-color 0.5s, color 0.5s' }}>
      ...
      <div className="absolute bottom-12 flex flex-col items-center gap-3" style={{ opacity: contentOpacity * 0.3, transition: 'opacity 1.5s ease 1.5s' }}>
...
const ProductCard = ({ product, onAddToCart, onOpenDressingRoom }: { product: Product; onAddToCart: (item: CartItem) => void; onOpenDressingRoom: (product: Product) => void; }) => {
  ...
  return (
    <div className="group relative bg-[#0a0a0a] border border-zinc-800/50 transition-all duration-700 hover:border-[#C9A962]/30"
      onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} style={{ transform: isHovered ? 'translateY(-4px)' : 'translateY(0)' }}>
      <div className="relative aspect-[3/4] overflow-hidden bg-zinc-900/30">
        <img src={product.images.back} alt={product.name}
        ...
        <div className={`absolute inset-0 flex items-center justify-center transition-all duration-500 ${isHovered ? 'opacity-100' : 'opacity-0'}`}>
          <div className="flex flex-col gap-3">
            <button onClick={() => onOpenDressingRoom(product)} className="px-6 py-3 bg-[#C9A962] text-black text-xs uppercase tracking-wider font-montserrat hover:bg-[#E8D5A3] transition-all duration-300">Try On</button>
            {product.price > 0 && <button onClick={handleAdd} className="px-6 py-3 bg-transparent border border-[#C9A962] text-[#C9A962] text-xs uppercase tracking-wider font-montserrat hover:bg-[#C9A962] hover:text-black transition-all duration-300">Quick Add</button>}
          </div>
        </div>
        ...
        <div className="flex gap-2 mb-3">
          {(['tshirt', 'hoodie'] as const).map((type) => (
            <button key={type} onClick={() => setSelectedType(type)}
              className={`px-3 py-1.5 text-[9px] uppercase tracking-wider transition-all duration-300 border ${selectedType === type ? 'bg-[#C9A962]/20 text-[#C9A962] border-[#C9A962]/40' : 'bg-zinc-900/50 text-zinc-600 border-transparent hover:text-zinc-400'}`}>
              {type}
            </button>
          ))}
        </div>
        ...
        <div className="flex gap-2 mb-4">
          {['black', 'white', 'charcoal'].map((color) => (
            <button key={color} onClick={() => setSelectedColor(color)}
              className={`w-6 h-6 rounded-sm transition-all duration-300 ${selectedColor === color ? 'ring-1 ring-[#C9A962] ring-offset-2 ring-offset-[#0a0a0a]' : ''}`}
              style={{ backgroundColor: color === 'charcoal' ? '#2a2a2a' : color, border: color === 'white' ? '1px solid #333' : 'none' }} />
          ))}
        </div>
        ...
        <div className="flex gap-2 mb-5">
          {['S', 'M', 'L', 'XL', 'XXL'].map((size) => (
            <button key={size} onClick={() => setSelectedSize(size)}
              className={`w-8 h-8 text-[11px] transition-all duration-300 ${selectedSize === size ? 'bg-[#C9A962]/20 text-[#C9A962] border border-[#C9A962]/40' : 'bg-zinc-900/50 text-zinc-600'}`}>
              {size}
            </button>
          ))}
        </div>

        <button onClick={handleAdd} disabled={product.price === 0}
...
const VirtualDressingRoom = ({ isOpen, onClose, initialProduct, onAddToCart }: { isOpen: boolean; onClose: () => void; initialProduct?: Product; onAddToCart: (item: CartItem) => void; }) => {
  ...
  return (
    <div className="fixed inset-0 z-[90] flex animate-fadeIn" style={{ background: 'rgba(1,1,1,0.98)' }}>
      <button onClick={onClose} className="absolute top-6 right-6 text-zinc-600 hover:text-zinc-400 z-50 transition-colors">
        <X className="w-7 h-7" />
      </button>
      ...
      <div className="flex-1 flex items-center justify-center p-8 relative overflow-hidden">
        ...
        <div style={{ transform: `scale(${zoom})`, transition: 'transform 0.3s ease' }}>
          <img src={view === 'back' ? selectedProduct.images.back : selectedProduct.images.front} alt={selectedProduct.name} className="max-h-[65vh] max-w-full object-contain opacity-50" />
        </div>
        ...
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex items-center gap-6 bg-zinc-900/50 backdrop-blur-md px-6 py-3 border border-zinc-800/50">
          <button onClick={() => setView(v => v === 'front' ? 'back' : 'front')} className="text-zinc-500 hover:text-[#C9A962] transition-colors"><RotateCw className="w-5 h-5" /></button>
          <div className="w-px h-6 bg-zinc-800" />
          <button onClick={() => setZoom(z => Math.max(0.7, z - 0.1))} className="text-zinc-500 hover:text-[#C9A962] transition-colors"><ZoomOut className="w-5 h-5" /></button>
          <span className="text-zinc-600 text-sm font-montserrat w-10 text-center">{Math.round(zoom * 100)}%</span>
          <button onClick={() => setZoom(z => Math.min(1.3, z + 0.1))} className="text-zinc-500 hover:text-[#C9A962] transition-colors"><ZoomIn className="w-5 h-5" /></button>
        </div>
      </div>
      ...
      <div className="w-[400px] bg-[#0a0a0a] border-l border-zinc-800/50 p-8 overflow-y-auto">
        ...
        <div className="mb-6">
          ...
          <div className="flex gap-3">
            {(['tshirt', 'hoodie'] as const).map((type) => (
              <button key={type} onClick={() => setSelectedType(type)}
                className={`flex-1 py-3 text-xs uppercase tracking-wider transition-all duration-300 border ${selectedType === type ? 'bg-[#C9A962]/10 text-[#C9A962] border-[#C9A962]/40' : 'bg-transparent text-zinc-600 border-zinc-800'}`}>
                {type}
              </button>
            ))}
          </div>
        ...
        <div className="mb-6">
          ...
          <div className="flex gap-3">
            {['black', 'white', 'charcoal'].map((color) => (
              <button key={color} onClick={() => setSelectedColor(color)}
                className={`w-12 h-12 rounded-sm transition-all duration-300 ${selectedColor === color ? 'ring-1 ring-[#C9A962] ring-offset-4 ring-offset-[#0a0a0a]' : ''}`}
                style={{ backgroundColor: color === 'charcoal' ? '#2a2a2a' : color, border: color === 'white' ? '1px solid #333' : 'none' }} />
            ))}
          </div>
        </div>
        ...
        <div className="mb-8">
          ...
          <div className="flex gap-3">
            {['S', 'M', 'L', 'XL', 'XXL'].map((size) => (
              <button key={size} onClick={() => setSelectedSize(size)}
                className={`w-11 h-11 text-sm transition-all duration-300 border ${selectedSize === size ? 'bg-[#C9A962]/10 text-[#C9A962] border-[#C9A962]/40' : 'bg-transparent text-zinc-600 border-zinc-800'}`}>
                {size}
              </button>
            ))}
          </div>
        ...
        <button onClick={handleAdd} disabled={selectedProduct.price === 0}
          ...
          <div className="grid grid-cols-4 gap-3">
            {PRODUCTS.map((p) => (
              <button key={p.id} onClick={() => setSelectedProduct(p)}
                className={`relative aspect-square overflow-hidden transition-all duration-300 border ${selectedProduct.id === p.id ? 'border-[#C9A962]/60' : 'border-zinc-800 opacity-50 hover:opacity-80'}`}>
...
const VirtualCloset = ({ isOpen, onClose, onOpenDressingRoom }: { isOpen: boolean; onClose: () => void; onOpenDressingRoom: (product: Product) => void; }) => {
  ...
  return (
    <div className="fixed inset-0 z-[90] flex flex-col animate-fadeIn" style={{ background: 'rgba(1,1,1,0.98)' }}>
      <div className="flex items-center justify-between p-8 border-b border-zinc-800/50">
        ...
        <button onClick={onClose} className="text-zinc-600 hover:text-zinc-400 transition-colors"><X className="w-7 h-7" /></button>
      </div>

      <div className="flex-1 overflow-y-auto p-8">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
          {PRODUCTS.map((product) => (
            <div key={product.id} className="group relative bg-zinc-900/30 border border-zinc-800/50 hover:border-[#C9A962]/30 transition-all duration-500 cursor-pointer"
              onClick={() => onOpenDressingRoom(product)}>
              <div className="aspect-[3/4] overflow-hidden">
                <img src={product.images.back} alt={product.name} className="w-full h-full object-cover opacity-40 group-hover:opacity-60 group-hover:scale-105 transition-all duration-700" />
              </div>
              <button onClick={(e) => { e.stopPropagation(); toggleFavorite(product.id); }}
                className={`absolute top-3 right-3 p-2 transition-all duration-300 ${favorites.includes(product.id) ? 'text-[#C9A962]' : 'text-zinc-700 hover:text-[#C9A962]'}`}>
    ...
    <div className="fixed inset-0 z-[80]">
      ...
      <div className="absolute right-0 top-0 bottom-0 w-full max-w-md bg-[#0a0a0a] border-l border-zinc-800/50 p-8 overflow-y-auto animate-slideInRight">
        <div className="flex justify-between items-center mb-10">
          ...
          <button onClick={onClose} className="text-zinc-600 hover:text-zinc-400 transition-colors"><X className="w-6 h-6" /></button>
        </div>
        ...
        {items.length === 0 ? (
          ...
          <>
              ...
              {items.map((item) => (
                <div key={`${item.id}-${item.size}-${item.color}-${item.type}`} className="flex gap-4 p-4 bg-zinc-900/30 border border-zinc-800/50">
                  ...
                  <div className="flex flex-col items-end justify-between">
                    <button onClick={() => onRemove(`${item.id}-${item.size}-${item.color}-${item.type}`)} className="text-zinc-700 hover:text-red-900 transition-colors"><Trash2 className="w-4 h-4" /></button>
                    <div className="flex items-center gap-2">
                      <button onClick={() => onUpdateQuantity(`${item.id}-${item.size}-${item.color}-${item.type}`, Math.max(0, item.quantity - 1))}><Minus className="w-4 h-4 text-zinc-600" /></button>
                      <span className="text-zinc-500 text-sm w-6 text-center">{item.quantity}</span>
                      <button onClick={() => onUpdateQuantity(`${item.id}-${item.size}-${item.color}-${item.type}`, item.quantity + 1)}><Plus className="w-4 h-4 text-zinc-600" /></button>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="border-t border-zinc-800/50 pt-8">
              ...
              <button className="w-full py-4 bg-[#C9A962]/10 text-[#C9A962] border border-[#C9A962]/40 font-cinzel text-sm tracking-wider uppercase hover:bg-[#C9A962]/20 transition-all duration-300">Checkout</button>
            </div>
          </>
        )}
      </div>
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 animate-fadeIn" style={{ background: 'rgba(1,1,1,0.98)' }}>
        <div className="bg-[#0a0a0a] border border-zinc-800/50 p-10 max-w-md w-full">
          ...
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleLogin()}
            placeholder="Password" className="w-full bg-zinc-900/50 border border-zinc-800 p-4 text-zinc-400 font-montserrat mb-4 focus:border-[#C9A962]/40 outline-none transition-colors" />
          {error && <p className="text-red-900 text-sm mb-4">{error}</p>}
          <button onClick={handleLogin} className="w-full py-4 bg-[#C9A962]/10 text-[#C9A962] border border-[#C9A962]/40 font-cinzel uppercase tracking-wider hover:bg-[#C9A962]/20 transition-all duration-300">Enter</button>
          <button onClick={onClose} className="w-full py-4 mt-3 text-zinc-700 font-montserrat text-sm hover:text-zinc-500 transition-colors">Return</button>
        </div>
      </div>
    );
  }
    ...
    <div className="fixed inset-0 z-[100] overflow-y-auto animate-fadeIn" style={{ background: 'rgba(1,1,1,0.98)' }}>
      <div className="max-w-6xl mx-auto p-8 md:p-12">
        <div className="flex justify-between items-center mb-12 border-b border-zinc-800/50 pb-6">
          ...
          <button onClick={onClose} className="text-zinc-700 hover:text-zinc-500 transition-colors"><X className="w-6 h-6" /></button>
        </div>
...
function App() {
  ...
  return (
    <div className="min-h-screen text-gray-300 font-montserrat" style={{ background: '#010101' }}>
      {/* HERO - Always rendered underneath */}
      <Hero onEnter={handleEnterShop} visible={showHero} />
      
      {/* SHOP - Rendered when user enters */}
      {showShop && (
        <>
          <Navbar 
      ...
      {/* SUMMONING SEAL - Rendered on top initially */}
      {showSeal && <SummoningSeal onComplete={handleSealComplete} />}
      
      {/* SMOKE EFFECT - Always on top */}
      <SmokeEffect />
    </div>
  );
}
